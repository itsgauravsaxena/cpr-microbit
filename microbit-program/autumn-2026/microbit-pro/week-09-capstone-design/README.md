...
- Everyone has *something* running, even if it's only half the idea.

## How it works

Every gadget is the same three-part sentence: **input → decision → output**. Write yours down before you code — 'when the light drops (input), if it's after bedtime (decision), play an alarm (output)'.

## Extra challenges
- Use two inputs at once (e.g. tilt *and* light).
- Add radio so two of your gadgets talk to each other.
- Design the sound and the LED animation, not just the logic.

## If it doesn’t work
- Idea too big? Cut it in half — get the smallest version working first, then add.
- Stuck? Steal a block pattern from an earlier week; that's what they're for.
